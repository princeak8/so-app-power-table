  ...
  <Transition name="modal">
    ...
    >
      ...
      >
        ...
        <!-- Body -->
        <div class="modal-body overflow-y-auto bg-gray-100 grid grid-cols-1 lg:grid-cols-3 gap-6 px-10 min-h-[70vh]">
          <!-- Overview Panel -->
          ...
          <div class="lg:col-span-2 bg-white rounded-lg p-5 shadow border border-gray-250 overflow-hidden px-6">
            ...
            <div v-if="station?.sections && station.sections.length > 0" class="flex flex-row flex-wrap gap-4 max-h-[60vh] overflow-y-auto pr-2">
              <div
                ...
                <div class="grid grid-cols-2 gap-2">
                  <div class="bg-gray-50 p-2 rounded border border-gray-100">
                    ...
                    <div class="text-md font-bold" :class="getUnitTextClass(unit)">
                      {{ formatValue(Math.abs(toNumber(unit.data.mw))) }} <span class="text-xs font-normal">MW</span>
                    </div>
                  </div>
                  <div class="bg-gray-50 p-2 rounded border border-gray-100">
                    <div class="text-xs font-medium text-gray-500 mb-1">Reactive Power</div>
                    <div class="text-md font-bold" :class="getUnitTextClass(unit)">
                      {{ formatValue(unit.data.mx) }} <span class="text-xs font-normal">MVar</span>
                    </div>
                  </div>
                  <div class="bg-gray-50 p-2 rounded border border-gray-100">
...
const totalPower = computed(() => {
  ...
  return props.station.sections
    ...
    .reduce((sum, unit) => sum + Math.abs(toNumber(unit.data.mw)), 0)
    .toFixed(1);
});
...
const totalReactivePower = computed(() => {
  if (!props.station?.sections) return 0;
  return props.station.sections
    .filter(unit => unit.data.mx && !isNaN(toNumber(unit.data.mx)))
    .reduce((sum, unit) => sum + toNumber(unit.data.mx), 0)
    .toFixed(1);
});
...
const getUnitStatus = (unit: any): string => {
  const power = Math.abs(parseFloat(unit.data.mw));
